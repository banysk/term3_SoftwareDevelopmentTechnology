...
public:
    ...
    hash_map_iterator(const pointer &ptr_) noexcept : ptr_(ptr_) {}
    ...
    hash_map_iterator operator++(int int_) {
        hash_map_iterator<value_type> it_ = *this;
        this->ptr_++;
        return it_;
    }
    friend bool operator==(const hash_map_iterator<ValueType> &left, const hash_map_iterator<ValueType> &right) {
...
public:
    ...
    hash_map_const_iterator(const pointer &ptr_) noexcept : ptr_(ptr_) {}
    reference operator*() const {
        return *ptr_;
    }
    pointer operator->() const {
    ...
    hash_map_const_iterator operator++(int int_) {
        hash_map_const_iterator<value_type> it_ = *this;
        this->ptr_++;
        return it_;
    }
    friend bool operator==(const hash_map_const_iterator<ValueType> &left, const hash_map_const_iterator<ValueType> &right) {
        return left.ptr_ == right.ptr_;
    }
    friend bool operator!=(const hash_map_const_iterator<ValueType> &left, const hash_map_const_iterator<ValueType> &right) {
        return left.ptr_ != right.ptr_;
...
public:
    ...
    explicit hash_map(size_type n) { // OK, TESTED
        ...
        if (n > 0) {
            ...
            is_placed_ = std::vector<bool>(n, 0);
            is_removed_ = std::vector<bool>(n, 0);
        }
        else {
            begin_ = nullptr;
        }
    ...
    hash_map(const InputIterator first, const InputIterator last, size_type n = 0); // fix me

    hash_map(const hash_map& hm) { // OK, ___
        load_factor_ = hm.load_factor_;
        size_ = hm.size_;
        capacity_ = hm.capacity_;
        alloc_ = hm.alloc_;
        is_placed_ = hm.is_placed_;
        is_removed_ = hm.is_removed_;
        begin_ = alloc_.allocate(capacity_);
        memcpy(begin_, hm.cbegin.operator->(), size_type * capacity_);
    };

    hash_map(hash_map&&) = default; // OK, ___

    explicit hash_map(const allocator_type& a) { // OK, ___
        load_factor_ = 1.0;
        size_ = 0;
        capacity_ = 0;
        begin_ = nullptr;
    ...
    hash_map(const hash_map& umap, const allocator_type& a) { // OK, ___
        this = copy(umap);
        alloc_ = a;
    }

    hash_map(hash_map&& umap, const allocator_type& a) { // OK, ___
        this = move(umap);
        alloc_ = a;
    }
     ...
     */
    hash_map(std::initializer_list<value_type> l, size_type n = 0); // fix me
    ...
    allocator_type get_allocator() const noexcept { // OK, ___
        return alloc_;
    }
    ...
    iterator end() noexcept { // OK, TESTED
        if (begin_ == nullptr) {
            ...
            return iterator(begin_ + (capacity_ - 1));
        }
    }
    ...
    const_iterator end() const noexcept { // OK, TESTED
        if (begin_ == nullptr) {
            ...
            return const_iterator(begin_ + (capacity_ - 1));
        }
    }
    ...
    */
    std::pair<iterator, bool> insert(const value_type& x) {
        int hash1 = std::hash<key_type>()(x.first);
        int hash2 = std::hash<mapped_type>()(x.second);
        int hash = (hash1 ^ hash2) % capacity_;
        *begin_ = x;
        return std::make_pair(iterator(&begin_[hash]), 1);
    }
    ...
    */
     ...
     *  in any way.  Managing the pointer is the user's responsibility.
     */
    void clear() noexcept { // OK, ___
        alloc_.deallocate(begin_, capacity_);
        is_placed_ = std::vector<bool>(capacity_, 0);
        is_removed_ = std::vector<bool>(capacity_, 0);
    }
